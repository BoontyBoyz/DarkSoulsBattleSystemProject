...
public class PlayerCamera : MonoBehaviour
{
    ...
    private void HandleRotations()
    {
        ...
        // NORMAL ROTATIONS
        // ROTATE LEFT AND RIGHT BASED ON HORIZONTAL MOVEMENT ON THE RIGHTSTICK/MOUSE
        leftAndRightLookAngle += (PlayerInputManager.instance.cameraHorizontalInput * leftAndRightRotationSpeed) * Time.deltaTime;
        // ROTATE UP AND DOWN BASED ON VERTICAL MOVEMENT ON THE RIGHTSTICK/MOUSE
    ...
    private void HandleCollisions()
    {
        ...
        if (Physics.SphereCast(cameraPivotTransform.transform.position, cameraCollisionRadius, direction, out hit, Mathf.Abs(targetCameraZPosition), collideWithLayers))
        {
            // IF THERE IS, WE GET OUR DISTANCE FROM IT
            float distanceFromHitObject = Vector3.Distance(cameraPivotTransform.position, hit.point);
            //WE THEN EQUATE OUR TARGET POSITION TO THE FOLLOWING
